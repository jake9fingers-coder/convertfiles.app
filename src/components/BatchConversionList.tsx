...
import { X, Play, RefreshCw, FileVideo, Music, CheckCircle, Plus, Trash2, ChevronDown, ChevronUp, Download } from 'lucide-react'
            ...
            {(!allDone || isExpanded || batch.length === 1) && (
                <div className="space-y-3">
                    {batch.map(item => {
                        ...
                        const outputExt = profile ? profile.outputExtension.toUpperCase() : ''
                        ...
                        const isAlreadyFormatError = (item.status === 'pending' || item.status === 'error') && item.mode && item.mode !== 'compress' && (
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            ...
                                            <div className="flex flex-col items-center gap-2 min-w-0 overflow-hidden">
                                                ...
                                                <div className="w-full aspect-square rounded-lg overflow-hidden bg-dark-50 border border-brand-200 flex items-center justify-center relative">
                                                    {item.mode === 'mp3' ? (
                                                        <div className="w-full h-full flex items-center justify-center bg-dark-50 text-dark-400">
                                                            <Music className="w-8 h-8" />
                                                        </div>
                                                    ) : item.result!.blob.type.startsWith('video/') ? (
                                                        <video
                                                            src={item.result!.url}
